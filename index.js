...
async function AlyaBotIncBot() {
	const {  saveCreds, state } = await useMultiFileAuthState(`./${sessionName}`)
    	const AlyaBotInc = AlyaBotIncConnect({
...
AlyaBotInc.ev.on('creds.update', await saveCreds)

    // Anti Call
    AlyaBotInc.ev.on('call', async (XeonPapa) => {
    let botNumber = await AlyaBotInc.decodeJid(AlyaBotInc.user.id)
    ...
    let XeonBlokMsg = await AlyaBotInc.sendTextWithMentions(XeonFucks.from, `*${AlyaBotInc.user.name}* can't receive ${XeonFucks.isVideo ? `video` : `voice` } call. Sorry @${XeonFucks.from.split('@')[0]} you will be blocked. If accidentally please contact the owner to be unblocked !`)
    AlyaBotInc.sendContact(XeonFucks.from, global.owner, XeonBlokMsg)
    await sleep(8000)
    await AlyaBotInc.updateBlockStatus(XeonFucks.from, "block")
    }
    }
    }
    })
...
await AlyaBotInc.readMessages([kay.key]) }
if (!AlyaBotInc.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
const m = smsg(AlyaBotInc, kay, store)
...
AlyaBotInc.sendTextWithMentions = async (jid, text, quoted, options = {}) => AlyaBotInc.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
...
return await AlyaBotInc.sendMessage(jid, { image: buffer, caption: caption, ...options }, { quoted })
}
...
AlyaBotInc.sendText = (jid, text, quoted = '', options) => AlyaBotInc.sendMessage(jid, { text: text, ...options }, { quoted })

AlyaBotInc.serializeM = (m) => smsg(AlyaBotInc, m, store)

AlyaBotInc.sendButtonText = (jid, buttons = [], text, footer, quoted = '', options = {}) => {
let buttonMessage = {
text,
footer,
buttons,
...
AlyaBotInc.sendMessage(jid, buttonMessage, { quoted, ...options })
}

AlyaBotInc.sendKatalog = async (jid , title = '' , desc = '', gam , options = {}) =>{
let message = await prepareWAMessageMedia({ image: gam }, { upload: AlyaBotInc.waUploadToServer })
...
return AlyaBotInc.relayMessage(jid, tod.message, {messageId: tod.key.id})
} 

AlyaBotInc.send5ButLoc = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
...
AlyaBotInc.relayMessage(jid, template.message, { messageId: template.key.id })
}

AlyaBotInc.sendButImg = async (jid, path, teks, fke, but) => {
...
AlyaBotInc.sendMessage(jid, fjejfjjjer, { quoted: m })
}
...
//AlyaBotInc.sendFile = async (jid, media, options = {}) => {
        //let file = await AlyaBotInc.getFile(media)
        ...
        //else type = "document"
        //return AlyaBotInc.sendMessage(jid, { [type]: file.data, ...options }, { ...options })
      //}
